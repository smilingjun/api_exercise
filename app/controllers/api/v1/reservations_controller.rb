class Api::V1::ReservationsController < ApiController
  def create
    ...
    @reservation =Reservation.new (:train_id => @train.id, :seat_number => pamras[:seat_number], :customer_name => params[:customer_name], :customer_phone => params[:customer_phone])

    if @reservation.save
      render :json => {:booking_code => @reservation.booking_code, :reservation_url => api_v1_reservation_url(@reservation.booking_code)}
    else
